...
/// Function to apply the theme, toggle logo visibility, and icon visibility
function applyTheme(themeName) {
  var body = document.body;
  var darkLogos = document.querySelectorAll('.white_logo');
  var whiteLogos = document.querySelectorAll('.dark_logo');
  var moonIcon = document.getElementById('moonIcon');
  var sunIcon = document.getElementById('sunIcon');

  // Remove all possible theme classes
  body.classList.remove('dark', 'semiDark', 'light');
  
  // Add the new theme class
  body.classList.add(themeName);

  // Update the logo visibility
  if (themeName === 'dark') {
  ...
  // Update currentTheme and set cookie
  currentTheme = themeName;
  setCookie('theme', themeName, 7); // Save theme in cookie for 7 days
}
...
// Initialization: Read theme from cookie and apply
document.addEventListener('DOMContentLoaded', function() {
  var savedTheme = getCookie('theme') || 'light'; // Default to light theme
